...
function getSortBy(query) {
    ...
    switch (query.orderBy) {
        case "mmr":
            sort_by = "mmrEquity:desc";
            break;

        case "nada":
            sort_by = "nadaEquity:desc";
            break;
...
async function GetAllFacets(dealerId = 1) {
    try {
        const dealerIds = await getActiveDealerIds(dealerId);
        const filterBy = `${statusFilter}&&numberOfPhotos:>9&&dealerId:[${dealerIds}]`;
...
async function GetUserLocations(zip) {
    ...
    const response = await Api.GetByParams(`https://old.thiscar.com/api/user/city/suggestions`, param);
    return response;
}
    ...
    try {
        ...
        const response = await search
            ...
            .search({
                q: "*",
                //sort_by,
                filter_by: `updatedAt: > ${twentyFourHoursAgo} && bodyType: == ${bodyType} && price:[${minPrice}..${maxPrice}] && vin: != ${vin} && price: > 9000 && numberOfPhotos:>9&&dealerId:[${dealerIds}] `,
                per_page: 25,
                page: 1
            });
