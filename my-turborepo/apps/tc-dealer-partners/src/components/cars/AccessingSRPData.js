...
import { GetAllCars, SearchCars, FilterListing } from "@/services/carService";
...
export default function SRP({
    ...
    const fetchData = async () => {
        ...
        if (
            ...
            let data = await FilterListing(
                appliedFilters,
                20,
                appConfig.SHOW_COMING_SOON == "true" ? showComingSoonCars : false,
                dealerConfig ? dealerConfig.chromeDealerId : 0
            );
            if (data != null) {
                carsInfo = data?.items;
                ...
                for (let i = 0; i < 3; i++) {
                    if (carsRes.length >= 8 * i + 5) {
                        const updatedCars = [
                            ...
                            { document: marketingDetails.marketingTiles[contactListIndex] },
                            carsRes[8 * i + 5],
        ...
        if (typeof url === "string") {
            ...
            if (sortParam) {
                if (sortParam === "priceHigh") {
                    ...
                    sortParams.orderAsc = false;
                } else if (sortParam === "mmr") {
                    sortParams.orderBy = "mmr";
                    sortParams.orderAsc = false;
                } else if (sortParam === "nada") {
                    sortParams.orderBy = "nada";
                    sortParams.orderAsc = false;
                }
            }
        ...
        const fetchData = async () => {
            if (linkAllCars != null && linkAllCars !== "") {
                ...
                if (data != null) {
                    ...
                    for (let i = 0; i < 3; i++) {
                        if (contactListIndex >= marketingDetails.marketingTiles.length) {
                            contactListIndex = 0;
                        }
                        if (carsRes.length >= 8 * i + 5) {
                            const updatedCars = [
            ...
            if (scrollTop + clientHeight >= scrollHeight - 500) {
                ...
                if (searchBarText == "" && !filterApplied) {
                    ...
                    if (pagination && dataRead) {
                        ...
                        const data = await FilterListing(
                            queryParams,
                            20,
                            appConfig.SHOW_COMING_SOON == "true" ? showComingSoonCars : false,
                            dealerConfig ? dealerConfig.chromeDealerId : 0
