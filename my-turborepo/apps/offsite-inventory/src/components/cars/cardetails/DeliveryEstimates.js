...
function DeliveryEstimates({ popupData, vin, dealerConfig, dealerZipCode }) {
    ...
    const UpdateDeliveryEstimates = async (zip = {}) => {
        ...
        const shippingResponse = await getDistanceInfo(dealerZipCode, zip);
        ...
        if (userDealersData) {
            if (userDealersData.chromeDealerId === dealerConfig.chromeDealerId) {
                ...
                setUserValidDealership(
                    userDealersData?.userSecondaryDealership?.some((user) => user.value === dealerConfig.chromeDealerId)
                );
            }
        }
    };
