...
function OtherVehicles({ similarCars, legalPopupData, dealerConfig }) {
    const headingOtherCars = "Other Vehicles You May Like";
    if (similarCars) {
        similarCars.items.forEach((item, index) => {
            if (!item.document.offsitePhotoUrls || item.document.offsitePhotoUrls.length < 10) {
                // If offsitePhotoUrls don't exist or have less than 10 items, remove this item from the array
...
function CarCard({ car, dealerConfig }) {
    const odoMeter = "miles";
    const currencyType = "$";
    const { dealer } = useParams();

    const handleSimilarCars = () => {
        window.open(`/${dealer}/${car.vin}`, "_self");
    };
    ...
    return (
        <div className="col-md">
            <div
                ...
                {car.offsitePhotoUrls && car.offsitePhotoUrls.length >= 10 && (
                    <Image
                        src={car.offsitePhotoUrls[0]}
                        alt="Car Image"
                        width={240}
                        height={170}
                ...
                <div className="card-body d-flex flex-column justify-content-between px-0 pb-0">
                    <div className="card-text-container">
                        ...
                        <p className="card-text-paragraph">
                            ...
                            {(car.price + (dealerConfig ? dealerConfig.retailMarkup : 0)).toLocaleString()}
                        </p>
                        <span>
                            <OtherVehiclesModalSVG />
                        </span>
                    </div>
                    <div className="card-footer text-muted fs-9">{GetDeliveryInfo(car, dealerConfig)}</div>
                </div>
            </div>
        </div>
    );
