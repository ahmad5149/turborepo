...
import { findYourRidePageContent } from "@/sanity/Sanity";
...
export async function generateMetadata({ searchParams }) {
    const findYourRideData = await findYourRidePageContent();
...
const CarsPage = async ({ params, searchParams }) => {
    const findYourRideData = await findYourRidePageContent();
    const allFacets = await GetAllFacets();
    ...
    return (
        <div className="text-center">
            {allFacets && findYourRideData && (
                <SRP
                    allFacets={allFacets}
                    contactDetails={findYourRideData.contactDetails}
